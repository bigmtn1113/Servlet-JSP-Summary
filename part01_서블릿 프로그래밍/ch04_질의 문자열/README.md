...
## 4.5. 한글 처리
만약 처음 한글을 입력하는 웹 브라우저에서 문자코드를 `KSC5601`로 설정하였다면,  
웹 브라우저에서는 `KSC5601` 문자코드를 사용하여 한글을 표현한다.

그런데 다음단계로 전달된 서버에서 만일 `UTF-8`로 문자코드를 설정했다면 웹 브라우저에서 `KSC5601`로 표현했던 한글은
서버에서는 `UTF-8` 문자코드를 사용하여 표현하게 된다.

즉, `같은 내용`의 한글이 서비스 처리를 하는 단계에서 매번 `다른 문자코드`를 사용하여 `재표현`되는 것이다.  
이것이 바로 한글이 깨지는 이유다.

깨진 한글을 복원하는 방법은 GET과 POST 방식이 다르다.  
GET 방식은 요청정보 헤더의 `URI`에, POST 방식은 요청정보 `몸체`에 포함되어 전달되기 때문이다.

### 4.5.1. POST 방식 한글 처리
POST 방식에서 한글 처리를 해주는 메소드는 HttpServletRequest의 상위 객체인 ServletRequest에서 제공하는
`setCharacterEncoding()` 메소드이다.

이 메소드는 인자값으로 지정된 문자코드를 사용하여 클라이언트가 전달한 요청정보 몸체에 있는 문자열들을 인코딩해주는 메소드이다.
```java
...
**참고**  
- EUC-KR
  `한글과 영문만` 지원하는 문자코드로서 한 글자를 표현할 때 2byte를 사용한다.  
  장점은 한글이 3byte로 표현되는 UTF-8보다 용량을 `적게` 차지한다는 것이고,  
  단점은 다국어 지원이 안 된다는 것이다.

- UTF-8  
  `전 세계` 모든 언어를 표현할 수 있어서 널리 사용된다.  
  영어는 1byte, 한글은 3byte로 표현되므로 한글은 EUC-KR에 비해 용량을 `더` 차지한다.

<br/>

### 4.3.2. GET 방식 한글 처리
질의 문자열이 요청정보 헤더의 URI에 포함되므로 `setCharacterEncoding()` 메소드를 사용할 수 없다.  
그래서 URI에 대해 인코딩 처리 작업을 해야 한다.

그 전에 생각해봐야 할 것이 있다.
- **클라이언트가 입력하는 질의 문자열의 인코딩 문자코드는 무엇인가?**  
  클라이언트가 입력한 문자열들을 인코딩 처리하는 방법은 두 가지이다.
  
  - 소스에서 직접 인코딩 문자코드를 지정하는 방법
    ```HTML
    <head>
      <meta charset="UTF-8">
    </head>
  ...
  - 이클립스에서 인코딩 문자를 지정하는 방법
  
- **서버에서 URI를 인코딩하는 문자코드는 무엇인가?**  
  클라이언트로부터 전달된 URI 문자열의 인코딩 작업은 서버에서 처리한다.
  
  서버마다 기본적으로 URI의 인코딩 문자코드가 정해져 있다.  
  만약 바꿀거면 `server.xml` 파일에서 인코딩 문자코드를 설정할 수 있다.
